...
def image_normalize(file_dir, args):
    file_id = file_dir.split('/')[-1].replace('.npz','')
    print('process '+file_id)
    assert os.path.exists(args.voxels_meta_dir), 'Can not find voxels analysis files'
    with open(args.voxels_meta_dir, 'rb') as voxels_file:
    ...
    np.savez_compressed(os.path.join(args.normalized_dir, file_id+'.npz'), data = new_data)
