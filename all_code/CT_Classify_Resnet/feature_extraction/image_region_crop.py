...
def crop_image(file_dir, args):
    file_id = os.path.basename(file_dir).replace('.npz','')
    ...
    #计算需要剪裁区域坐标
    crop_boxes, new_mask = compute_crop_boxes_coord(mask, args)
    # 提取mask前景对应的image
    new_image = np.full_like(image, fill_value = np.min(image))
    new_image[new_mask > 0] = image[new_mask > 0]
    # 将image向外扩边，以防止剪裁时超边缘
    new_image = image_padding(new_image, args)
    # 由于原始image的H,Y,X轴向两侧均pading,crop_boxes坐标需要重新计算
    crop_boxes = boxes_padding(crop_boxes, args)
    # 按crop坐标剪裁image
    ...
    '''
    np.savez_compressed(os.path.join(args.cropped_dir, file_id+'.npz'), data = image_cropped)
    print('cropped %s'%file_id)

def crop_tumor_image(file_dir, args):
    file_id = os.path.basename(file_dir).replace('.npz','')
    assert os.path.exists(file_dir), 'Can not find file %s'%(file_id)
    # load image & mask
    all_data = np.load(file_dir)['data']
    ...
    #assert 2 in mask, 'The label 2 not in mask of %s'%(file_id)
    #计算需要剪裁区域坐标
    crop_boxes, new_mask = compute_crop_tumor_boxes_coord(mask, args)
    # 提取mask前景对应的image
    new_image = np.full_like(image, fill_value = np.min(image))
    new_image[new_mask > 0] = image[new_mask > 0]
    # 将image向外扩边，以防止剪裁时超边缘
    ...
    for i,img in enumerate(image_cropped):
        ...
        plt.savefig('%s.png'%(str(i)))
    '''
    np.savez_compressed(os.path.join(args.tumor_cropped_dir, file_id+'.npz'), data = image_cropped)
    print('cropped %s'%file_id)



def compute_crop_boxes_coord(mask, args):
    # 计算包含肿瘤的肾
    ...
    # 计算需剪切box的坐标
    crop_boxes = [coord - args.crop_size[i]/2 for i,coord in enumerate(center_point)] + [coord + args.crop_size[i]/2 for i,coord in enumerate(center_point)]
    return crop_boxes, new_mask

def compute_crop_tumor_boxes_coord(mask, args):
    # 计算包含肿瘤的肾
    tumor_mask = np.zeros_like(mask)
    tumor_mask[mask == 2] = 1
    ...
    # 计算需剪切box的坐标
    crop_boxes = [coord - args.crop_tumor_size[i]/2 for i,coord in enumerate(center_point)] + [coord + args.crop_tumor_size[i]/2 for i,coord in enumerate(center_point)]
    return crop_boxes, new_mask


def image_padding(array, args):
    '''
    return a padded data array with cval
    在图像三个维度的最外边缘填充，值为cval
    '''
    pad_H, pad_Y, pad_X = [int(i/2) for i in args.crop_size]
...
if __name__ == '__main__':
    # for debug
    file_dir = '/share/service04/wukai/CT_image/images_normalized/TCGA-BP-4762_8.npz'